...
class BashOrgRu {
    ...
    function openHandler(& $parser,$name,$attrs) {
        if ($name == 'html') RSSWriter::beginRSS('windows-1251', 'Bash.Org.Ru', $this->url, 'Цитатник Рунета', 'ru');
        
        if (!$this->insideItem
        and $name == 'td' 
        and $attrs['align'] == 'left') {
            RSSWriter::beginItem();
        ...
        and $name == 'a') {
            RSSWriter::beginLink();
            RSSWriter::putLink('http://bash.org.ru/' . $attrs['href']);
            RSSWriter::endLink();
